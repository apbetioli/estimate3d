...
Not completely, but possible to a degree. Starting from version 0.76, PAR supports an inputFileName filter mechanism, which can be used to implement source obfuscators (or even product activation schemes).
